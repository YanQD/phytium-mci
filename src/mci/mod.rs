...
impl MCI {
    ...
    pub fn set_pio_mode(&mut self) {
        ...
        self.reg
            .modify_reg(|reg| !FsdifCtrl::USE_INTERNAL_DMAC & !FsdifCtrl::INT_ENABLE & reg);
    }
    ...
    pub fn clk_freq_set(&mut self, clk_hz: u32) -> FsdifResult {
        ...
        if clk_hz > 0 && self.config.get_tuning as usize != default_tuning as usize {
            ...
            /* set clock divider */
            self.reg
                .write_reg(FsdifClkDiv::from_bits_truncate(target_timing.clk_div));
            self.reg
                .write_reg(FsdifEnableShift::from_bits_truncate(target_timing.shift));
            info!(
    ...
    pub fn select_bus_timing(&mut self) -> FsdifResult {
        ...
        match self.select_function(GroupNum::TimingMode, TimingFunctionNum::SDR25HighSpeed) {
            Err(_) => {
    ...
    pub fn select_function(&self, group: GroupNum, function: TimingFunctionNum) -> FsdifResult {
        ...
        // todo 这里暂时不需要写，后续需要写上，因为需要相应的结构体
        /* CMD6 */
        ...
        self.convert_data_to_little_endian(
            &mut status,
            5,
            DataPacketFormat::DataPacketFormatMSBFirst,
        );
        /*
    ...
    pub fn prob_bus_voltage(&mut self) -> FsdifResult {
        ...
        application_command41_argument |=
            SdOcrFlag::VDD_29_30 | SdOcrFlag::VDD_32_33 | SdOcrFlag::VDD_34_35;
        /*
        ...
        /* card go idle */
        self.go_idle()?; // CMD0
        match self.send_interface_condition() {
            /* CMD8 */
            Ok(_) => {
                /* SDHC or SDXC card */
    ...
    pub fn update_exteral_clk(&self, uhs_reg: FsdifClkSrc) -> FsdifResult {
        ...
        self.reg.wait_for(
            |reg| (FsdifClkSts::READY & reg).bits() != 0,
            Duration::from_millis((FSDIF_TIMEOUT / 100).into()),
            Some(100),
        )?;
        Ok(())
    }
    ...
    pub fn init_external_clk(&self) -> FsdifResult {
        let reg_val = FsdifClkSrc::uhs_reg(0, 0, 0x5) | FsdifClkSrc::UHS_EXT_CLK_ENA;
    ...
    */
    pub fn fifoth_set(&self, trans_size: FsdifFifoThDmaTransSize, rx_wmark: u32, tx_wmark: u32) {
        let trans_size: u32 = trans_size.into();
        let val = (FsdifFifoTh::DMA_TRANS_MASK & (trans_size << 28).into())
    ...
    */
    pub fn uhs_reg_set(&self, drv_phase: u32, samp_phase: u32, clk_div: u32) {
        self.reg
            .modify_reg(|reg| FsdifClkSrc::uhs_reg(drv_phase, samp_phase, clk_div));
    }
    ...
    pub fn ctrl_reset(&self, reset_bits: FsdifCtrl) -> FsdifResult {
        self.reg.modify_reg(|reg| reset_bits | reg);
        self.reg.wait_for(
            |reg| (reset_bits & reg).bits() == 0,
            Duration::from_millis((FSDIF_TIMEOUT / 100).into()),
            Some(100),
        )?;
        ...
        if reset_bits.contains(FsdifCtrl::FIFO_RESET) {
            self.reg.wait_for(
                |reg| (FsdifStatus::FIFO_EMPTY & reg).bits() != 0,
                Duration::from_millis((FSDIF_TIMEOUT / 100).into()),
                Some(100),
            )?;
        }
    ...
    pub fn send_private_cmd(&self, cmd: FsdifCmd, arg: u32) -> FsdifResult {
        self.reg.wait_for(
            |reg| (FsdifStatus::DATA_BUSY & reg).bits() == 0,
            Duration::from_millis((FSDIF_TIMEOUT / 100).into()),
            Some(100),
        )?;
    ...
    pub fn descriptor_set(&self, desc: u32) {
        ...
        self.reg
            .write_reg(FsdifDescListAddrL::from_bits_truncate(desc));
    }
    ...
    pub fn busy_card_reset(&self) -> FsdifResult {
        self.reg.modify_reg(|reg| FsdifCtrl::CONTROLLER_RESET | reg);
        self.reg.wait_for(
            |reg| {
                self.reg.modify_reg(|reg| FsdifCtrl::CONTROLLER_RESET | reg);
    ...
    pub fn reset(&self) -> FsdifResult {
        ...
        /* set card threshold */
        self.reg.write_reg(
            FsdifCardThrctl::CARDRD | FsdifFifoDepth::Depth8.card_thrctl_threshold().into(),
        );
        info!(
            "card threshold set to 0x{:x}",
            (FsdifCardThrctl::CARDRD | FsdifFifoDepth::Depth8.card_thrctl_threshold().into())
                .bits()
        );
        ...
        if self.config.non_removable {
            self.reg.modify_reg(|reg| FsdifCardReset::ENABLE | reg);
        } else {
            self.reg.modify_reg(|reg| !FsdifCardReset::ENABLE & reg);
        }
        info!(
            "card reset to 0x{:x}",
            self.reg.read_reg::<FsdifCardReset>()
        );
        debug!("card reset success");
        /* clear interrupt status */
        self.reg.write_reg(FsdifInt::empty());
        info!(
            "clear interrupt status to 0x{:x}",
            self.reg.read_reg::<FsdifInt>()
        );
        let reg_val = self.reg.read_reg::<FsdifRawInts>();
        self.reg.write_reg(reg_val);
        info!(
            "clear interrupt status to 0x{:x}",
            self.reg.read_reg::<FsdifRawInts>()
        );
        self.reg.write_reg(FsdifDmacIntEn::empty());
        info!(
            "clear interrupt status to 0x{:x}",
            self.reg.read_reg::<FsdifDmacIntEn>()
        );
        let reg_val = self.reg.read_reg::<FsdifDmacStatus>();
        self.reg.write_reg(reg_val);
        info!(
            "clear interrupt status to 0x{:x}",
            self.reg.read_reg::<FsdifDmacStatus>()
        );
        debug!("clear interrupt status success");
        /* enable card detect interrupt */
        if !self.config.non_removable {
            self.reg.modify_reg(|reg| FsdifInt::CD_BIT | reg);
        }
        info!(
            "enable card detect interrupt to 0x{:x}",
            self.reg.read_reg::<FsdifInt>()
        );
        debug!("enable card detect interrupt success");
        /* enable controller and internal DMA */
        self.reg
            .modify_reg(|reg| FsdifCtrl::INT_ENABLE | FsdifCtrl::USE_INTERNAL_DMAC | reg);
        info!(
            "enable controller and internal DMA to 0x{:x}",
            self.reg.read_reg::<FsdifCtrl>()
        );
        ...
        info!(
            "set data and resp timeout to 0x{:x}",
            self.reg.read_reg::<FsdifTimeout>()
        );
...
impl MCI {
    ...
    pub fn decode_csd(&mut self, rawcsd: &[u32]) {
        ...
        if self.sd_reg.csd.csd_structure == 0 {
            ...
            self.block_count =
                (self.sd_reg.csd.device_size + 1) << (self.sd_reg.csd.device_size_multiplier + 2);
    ...
    pub fn decode_status(&mut self, src: &[u32]) {
        ...
        self.sd_reg.status.erase_size =
            (((src[2] & 0x000000FF) << 8) | ((src[3] & 0xFF000000) >> 24)) as u16;
...
impl MCI {
    ...
    pub fn switch_function(
        ...
        let mut cmd_data = FSdifCmdData {
            ...
            response: [0; 4],
            flag: CmdFlag::READ_DATA
                | CmdFlag::EXP_DATA
                | CmdFlag::EXP_RESP
                | CmdFlag::NEED_RESP_CRC,
            data: FsdifBuf {
                buf: status,
                buf_dma: 0,
                blksz: 64,
    ...
    pub fn select_card(&self, relative_address: u32, is_selected: bool) -> FsdifResult {
        let mut cmd_data = FSdifCmdData {
            ...
            response: [0; 4],
            flag: if is_selected {
                CmdFlag::EXP_RESP | CmdFlag::NEED_RESP_CRC
            } else {
                CmdFlag::empty()
            },
            data: FsdifBuf {
...
impl MCI {
    ...
    pub fn pio_transfer(&self, cmd_data: &FSdifCmdData) -> FsdifResult {
        ...
        self.reg
            .modify_reg(|reg| !FsdifCtrl::USE_INTERNAL_DMAC & reg);
        self.ctrl_reset(FsdifCtrl::FIFO_RESET)?;
        self.reg.modify_reg(|reg| !FsdifBusMode::DE & reg);
        /* write data */
    ...
    pub fn poll_wait_pio_end(&self, cmd_data: &mut FSdifCmdData) -> FsdifResult {
        ...
        self.reg.wait_for(
            |reg| (FsdifRawInts::CMD_BIT & reg).bits() != 0,
            Duration::from_millis((FSDIF_TIMEOUT / 100).into()),
            Some(100),
        )?;
        ...
        if read {
            ...
            if let Err(_) = self.reg.wait_for(
                |reg| {
...
impl MCI {
    ...
    pub fn transfer_cmd(&self, cmd_data: &FSdifCmdData) -> FsdifResult {
        ...
        self.interrupt_mask_set(
            FsDifIntrType::GeneralIntr,
            FsdifInt::INTS_CMD_MASK.bits(),
            true,
        );
    ...
    pub fn get_cmd_response(&self, cmd_data: &mut FSdifCmdData) -> FsdifResult {
        ...
        self.interrupt_mask_set(
            FsDifIntrType::GeneralIntr,
            (FsdifInt::INTS_CMD_MASK | FsdifInt::INTS_DATA_MASK).bits(),
            false,
        );
        self.interrupt_mask_set(
            FsDifIntrType::DmaIntr,
            FsdifDmacIntEn::INTS_MASK.bits(),
            false,
        );
        info!("cmd send done ...");
        Ok(())
...
impl MCI {
    ...
    pub fn dump_register(&self) {
        ...
        warn!(
            "descaddrl: 0x{:x}",
            self.reg.read_reg::<FsdifDescListAddrL>()
        );
        warn!(
            "descaddrh: 0x{:x}",
            self.reg.read_reg::<FsdifDescListAddrH>()
        );
        warn!("dmacstatus: 0x{:x}", self.reg.read_reg::<FsdifDmacStatus>());
        warn!("dmacinten: 0x{:x}", self.reg.read_reg::<FsdifDmacIntEn>());
        warn!(
            "curdescaddrl: 0x{:x}",
            self.reg.read_reg::<FsdifCurDescAddrL>()
        );
        warn!(
            "curdescaddrh: 0x{:x}",
            self.reg.read_reg::<FsdifCurDescAddrH>()
        );
        warn!(
            "curbufaddrl: 0x{:x}",
            self.reg.read_reg::<FsdifCurBufAddrL>()
        );
        warn!(
            "curbufaddrh: 0x{:x}",
            self.reg.read_reg::<FsdifCurBufAddrH>()
        );
        warn!(
            "card_thrctl: 0x{:x}",
            self.reg.read_reg::<FsdifCardThrctl>()
        );
